...
import { useFinancialRatioJudgment, useFinancialAnalysisDetails } from "@/lib/api/hooks";

// ── 2. 재무 비율 판정 ─────────────────────────────────────
function RatioJudgment({ code }: { code: string }) {
  const { data, isLoading } = useFinancialRatioJudgment(code);
...
// ── 분석 그룹 (안정성/수익성/성장성/활동성) ───────────────
function AnalysisGroups({ code }: { code: string }) {
  const { data, isLoading } = useFinancialAnalysisDetails(code);
  ...
  const groups = [
    {
      num: 3, title: "안정성 분석", description: SECTION_DESC.stability,
      subs: [
        { sub: "3.1. 유동성 분석", rows: data.liquidity },
        { sub: "3.2. 레버리지 분석", rows: data.leverage },
      ],
    },
    {
      num: 4, title: "수익성 분석", description: SECTION_DESC.profitability,
      subs: [
        { sub: "4.1. 투자수익성 분석", rows: data.investProfit },
        { sub: "4.2. 판매마진 분석", rows: data.salesMargin },
      ],
    },
    { num: 5, title: "성장성 분석", description: SECTION_DESC.growth, subs: [{ sub: "", rows: data.growth }] },
    { num: 6, title: "활동성 분석", description: SECTION_DESC.activity, subs: [{ sub: "", rows: data.activity }] },
  ];
  ...
  return (
    <>
      {groups.map((g) => (
        <section key={g.num}>
          <SectionTitle num={g.num} description={g.description}>{g.title}</SectionTitle>
...
export default function FinancialSection({ code }: { code: string }) {
  return (
    <div className="flex flex-col" style={{ gap: 64 }}>
      <FinancialStatusBlock code={code} num={1} title="재무 상황" description={SECTION_DESC.situation} />
      <RatioJudgment code={code} />
      <AnalysisGroups code={code} />
    </div>
  );
}
